...
<TABLE class='adm_cate table table-striped'>
  ...
  <c:forEach var="category" items="${list }" varStatus="status">
    <c:set var="cg_no" value="${category.g_cg_no }"/>
    <c:set var="cg_name" value="${category.g_name }"/>
    <c:set var="cateno" value="${category.cateno }"/>
    <c:set var="name" value="${category.name }"/>
    <c:set var="visible" value="${category.visible }"/>
    <c:set var="seqno" value="${category.seqno }"/>
    <c:set var="rdate" value="${category.rdate }"/>
    
    <tr>
      <td class="td_bs">${seqno }</td>
      <td class="td_bs">${cg_name }</td>
      <td class="td_bs_left"><a href="../ebook/list.do?cateno=${cateno }">${name }</a></td>
      <td class="td_bs">${rdate.substring(0, 10) }</td>
      <td class="td_bs">
        <c:choose>
          <c:when test="${visible == 'Y'}">
            <A href="./update_visible.do?cateno=${cateno }&visible=N&cg_no=${cg_no}">
              <i class="fas fa-eye"></i>
            </A>
          </c:when>
          <c:otherwise>
            <A href="./update_visible.do?cateno=${cateno }&visible=Y&cg_no=${cg_no}">
      ...
      <td class="td_bs">
        <a href="./read_update.do?cateno=${cateno }&cg_no=${cg_no}">
          <i class="fas fa-pencil-alt"></i>
        </a>
        <a href="./read_delete.do?cateno=${cateno }&cg_no=${cg_no}">
          <i class="fas fa-trash-alt"></i>
        </a> 
        <a href="./update_seqno_up.do?cateno=${cateno }&cg_no=${cg_no}">
          <i class="fas fa-arrow-up"></i>
        </a>
        <a href="./update_seqno_down.do?cateno=${cateno }&cg_no=${cg_no}">
