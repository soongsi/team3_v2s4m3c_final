...
<DIV class='title_line'>설문조사 - 삭제 </DIV>
  
  <div> 
    <form name='frm' method='post' action='./delete.do' class="form-horizontal">
    <div class="msg_warning">삭제하시면 복구 할 수 없습니다.</div>
     <input type='hidden' name='survey_no' id='survey_no' value='${surveyVO.survey_no }'>
     
     <div style="width: 80%;margin: 10px auto;">
      <div class="form-group col-md-1 float-left text-center">
         <label class="control-label">순서</label>
         <div>${surveyVO.seqno }</div>
      </div>
      
      <div class="form-group col-md-4 float-left text-center">
        <input type="hidden" name="survey_no" value="${surveyVO.survey_no}">
         <label class="control-label">제목</label>
         <div>${surveyVO.title }</div>
      </div>
      
      <div class="form-group col-md-2 float-left">
         <label class="control-label">시작일</label>
         <div>${surveyVO.startdate }</div>
      </div>  
      <div class="form-group col-md-2 float-left">
         <label class="control-label">종료일</label>
         <div>${surveyVO.enddate } </div>
      </div>   
      <div class="form-group col-md-1 float-left">
         <label class="control-label">상태</label>
         <div>${surveyVO.continueyn }</div>
      </div>   
    
     <div class="col-md-2 float-left text-center" style="height: 50px;display: table;">
